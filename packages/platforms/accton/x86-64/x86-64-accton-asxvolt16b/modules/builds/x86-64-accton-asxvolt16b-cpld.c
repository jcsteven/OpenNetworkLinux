...
#define DRVNAME "asxvolt16b_cpld"
...
static int asxvolt16b_cpld_probe(struct platform_device *pdev);
static int asxvolt16b_cpld_remove(struct platform_device *pdev);
...
struct asxvolt16b_cpld_data *data = NULL;

static struct platform_driver asxvolt16b_cpld_driver = {
    .probe      = asxvolt16b_cpld_probe,
    .remove     = asxvolt16b_cpld_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static struct asxvolt16b_cpld_data *asxvolt16b_cpld_update_device(void)
{
	int status = 0;
#if 0
	if (time_before(jiffies, data->last_updated + HZ * 5) && data->valid) {
		dev_err(&data->pdev->dev, "%s:%i-Info: asxvolt16b_cpld_update_device-Exit-1.\n", __FUNCTION__,__LINE__);
		return data;
	}
#endif
	data->valid = 0;
...
int asxvolt16_cpld_read(unsigned short cpld_addr, u8 reg)
{
	...
	data = asxvolt16b_cpld_update_device();
	if (!data->valid) {
...
static int __init asxvolt16b_cpld_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct asxvolt16b_cpld_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&asxvolt16b_cpld_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&asxvolt16b_cpld_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit asxvolt16b_cpld_exit(void)
{
...
module_init(asxvolt16b_cpld_init);
module_exit(asxvolt16b_cpld_exit);
